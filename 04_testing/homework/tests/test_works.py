...
class TestMethodRequestValidation:
    ...
    @pytest.mark.parametrize("bad_field", ["login", "token", "arguments", "method"])
    ...
    @pytest.mark.parametrize(
        "bad_field, bad_field_value, error_msg",
        [
...
class TestClientsInterestsRequestValidate:
    @classmethod
    def get_valid_args(cls):
        return {"client_ids": [1, 2], "date": "01.01.2017"}
...
class TestMethodHandler:
    ...
    def get_response(cls, request, store=None):
        return api.method_handler(
            request={"body": request, "headers": {}}, context={}, store=store
        )
    ...
    def get_valid_args(cls, is_admin=True, method="online_score"):
        ...
        result_args = TestMethodRequestValidation.get_valid_args(is_admin, method)
    ...
    def test_bad_fields(self):
        _, code = self.get_response({"account": 123})
        assert code == api.INVALID_REQUEST
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            ...
            "empty_admin_token",
            "bad_admin_token",
            "admin_token_for_simple_user",
            "simple_user_token_for_admin",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            "online_score_empty_args",
            "clients_interests_empty_args",
            "online_score_bad_args",
            "clients_interests_bad_args",
        ],
    )
